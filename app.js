...
/* ==================== DELETE UX + COBERTURA ==================== */
function setDeleteModeUI(on) {
  const container = map?.getContainer?.();
  if (container) container.classList.toggle("delete-mode", !!on);
}

function setSelectedDeleteTarget(target) {
  // target: { kind:'custom'|'nucleo', id, circle, name }
  // limpiar estilo anterior
  if (selectedDeleteTarget?.circle && selectedDeleteTarget.circle.setStyle) {
    // restaurar según tipo/estado
    restoreCircleStyle(selectedDeleteTarget);
  }
  selectedDeleteTarget = target;
  if (selectedDeleteTarget?.circle?.setStyle) {
    selectedDeleteTarget.circle.setStyle({ color: "#7f1d1d", fillColor: "#7f1d1d", weight: 3, fillOpacity: 0.22 });
  }
  if (deleteMode) {
    showNotification("⌨️ Presiona Delete/Supr para eliminar el buffer seleccionado", "info");
  }
}

function restoreCircleStyle(target) {
  if (!target?.circle?.setStyle) return;
  if (target.kind === "custom") {
    // en modo delete se mantiene rojo, fuera púrpura
    if (deleteMode) target.circle.setStyle({ color: "#f85149", fillColor: "#f85149", weight: 2, fillOpacity: 0.15 });
    else target.circle.setStyle({ color: "#a371f7", fillColor: "#a371f7", weight: 2, fillOpacity: 0.15 });
  } else {
    // nucleo editable
    target.circle.setStyle({ color: "#58a6ff", fillColor: "#58a6ff", weight: 2, fillOpacity: 0.12 });
  }
}

function bindDeleteKeyListener() {
  if (_deleteKeyListenerBound) return;
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (!selectedDeleteTarget) {
      showNotification("Selecciona un buffer primero (click)", "error");
      return;
    }
    if (selectedDeleteTarget.kind !== "custom") {
      showNotification("⚠️ Los buffers de núcleo no se pueden eliminar, solo mover", "error");
      return;
    }
    const ok = confirm(`¿Eliminar "${selectedDeleteTarget.name || 'Buffer'}"?`);
    if (!ok) return;
    deleteCustomBuffer(selectedDeleteTarget.id);
    selectedDeleteTarget = null;
    // Limpieza de vacíos y actualización de cobertura
    sweepRemoveEmptyCustomBuffers();
    updateCoverageOverlay();
  }, { passive: true });
}

/**
 * Elimina automáticamente buffers personalizados vacíos (sin instituciones dentro).
 * Criterio: calculateBufferMetricsDetailed(...).iesCount === 0
 */
function sweepRemoveEmptyCustomBuffers() {
  if (!customBuffers?.length) return 0;
  let removed = 0;
  const copy = [...customBuffers];
  copy.forEach(b => {
    const pos = b.circle.getLatLng();
    const m = calculateBufferMetricsDetailed(pos, BUFFER_RADIUS_M);
    if ((m?.iesCount || 0) === 0) {
      deleteCustomBuffer(b.id);
      removed++;
    }
  });
  if (removed > 0) showNotification(`🧹 Limpieza: ${removed} buffers vacíos eliminados`, "info");
  return removed;
}

/* ===== Rompecabezas (cobertura territorial por ajuste manual) ===== */
function updateCoverageOverlay() {
  // La capa coverage solo dibuja “overlay”: verde ajustado, rojo pendiente.
  layers.coverage.clearLayers();

  const allCircles = [];
  editableBuffers.forEach((data, ni) => {
    if (!data?.circle) return;
    // solo si está seleccionado/visible
    allCircles.push({ circle: data.circle, adjusted: !!data.adjusted, kind: "nucleo" });
  });
  customBuffers.forEach(b => {
    if (!b?.circle) return;
    allCircles.push({ circle: b.circle, adjusted: !!b.adjusted, kind: "custom" });
  });
  ...
  allCircles.forEach(item => {
    ...
    const r = c.getRadius();
    total++;
    if (item.adjusted) adjusted++;
    const color = item.adjusted ? "#3fb950" : "#f85149";
  ...
  // progreso
  const el = document.getElementById("coverageProgress");
  if (el) {
    const pct = total ? ((adjusted / total) * 100).toFixed(1) : "0.0";
    el.textContent = `${adjusted}/${total} buffers ajustados – ${pct}%`;
  }
}
...
function createCustomBuffer(lat, lng) {
  ...
  if ((metrics.iesCount || 0) === 0) {
    // Limpieza automática: no dejar buffers vacíos
    setTimeout(() => {
      deleteCustomBuffer(buffer.id);
      selectedDeleteTarget = null;
      updateCoverageOverlay();
      showNotification("🧹 Buffer vacío eliminado automáticamente", "info");
    }, 50);
    return;
  }
...
function makeBufferDraggable(circle, data, isCustom, ni = null) {
  ...
  circle.on('mousedown', function(e) {
    ...
    const onUp = () => {
      ...
      data.currentPos = pos;
      data.adjusted = true;
      if (circle) circle._adjusted = true;
...
function processData(data) {
  ...
  // Limpieza automática de buffers vacíos + overlay rompecabezas
  sweepRemoveEmptyCustomBuffers();
  updateCoverageOverlay();
...
function setupControls() {
  ...
  ["toggleBuffers", "toggleConnections", "toggleNucleos", "toggleSatellites", "toggleCoverage"].forEach((id, i) => {
    ...
    if (el) el.addEventListener("change", (e) => { const layer = [layers.buffers, layers.connections, layers.nucleos, layers.satellites, layers.coverage][i]; e.target.checked ? map.addLayer(layer) : map.removeLayer(layer); });
  });

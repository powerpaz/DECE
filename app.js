...
function loadCSV() {
  ...
  Papa.parse('DECE_CRUCE_X_Y_NUC_SAT.csv', {
    ...
    complete: (results) => {
      ...
      if (issues.length) console.warn('Column issues:', issues);
...
function resolveColumnIndexes(headerRow) {
  ...
  const idx = {
    ...
    cod: findOne(['cod_gdece', 'cod gdece', 'cod_gdece ']),
    name: findOne(['nombre_institución', 'nombre_institucion', 'nombre de la institución', 'nombre institucion', 'nombre', 'Nombre_Institución']),
    ...
    profs: findOne(['po_profdece', 'profdece', 'PO_ProfDECE', 'profesionales', 'nro profesionales'])
  };
...
function processData(data) {
  ...
  drawBuffersInBatches(nucleos, 0, 450, () => {

    // 2) Satélites por lotes + conexiones estáticas (canvas)
    ...
    assignSatellitesInBatches(satellites, grid, nucleoStats, 0, 280, (satCovered) => {
      ...
      // 3) Núcleos por lotes + cálculo top absorción
      ...
      drawNucleosInBatches(nucleos, nucleoStats, topKeys, 0, 450, () => {
        ...
        // 4) “Tipo redes”: halo + conexiones animadas SOLO top núcleos (SVG)

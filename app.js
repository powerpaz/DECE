...
function buildBuffersBRowsForExport() {
  try {
    ...
    // Alias por bufferId (B#)
    const aliasById = new Map();
    list.forEach(ob => aliasById.set(ob.bufferId, String(ob.alias || ob.bufferId)));

    // ===== ESPEJO + CALIDAD (sin duplicados por AMIE) =====
    // Regla: un AMIE fiscal NO puede exportarse en más de un B#.
    // Si el punto cae en varios buffers (solape), se asigna al buffer cuyo centro esté más cerca.
    // Además, si el CSV trae filas duplicadas del mismo AMIE, igual se exporta UNA sola vez.
    const R = Number(window.BUFFER_RADIUS_METERS || BUFFER_RADIUS_METERS || 7500); // metros
    ...
    // Distancia en metros (Leaflet si está, fallback haversine)
    ...
    // AMIE -> {chosenBufferId, chosenDist, casos}
    const assignmentByAmie = new Map();

    // Satélites dentro de buffers B# según estado del mapa
    // satInsideOrphan está indexado por si (índice en globalData.satellites)
    const inside = orphanBufferCoverageState?.satInsideOrphan;
    const sats = globalData?.satellites || [];

    if (!inside || !sats.length) return rows;

    // Recorrer satélites que el mapa considera dentro de buffers B#
    inside.forEach((_bid, si) => {
      const s = sats[si];
      const amie = String(s?.amie || '').trim();
      const lat = Number(s?.lat);
      const lng = Number(s?.lng);
      if (!amie || !Number.isFinite(lat) || !Number.isFinite(lng)) return;

      // Candidatos reales (geometría): buffers cuyo círculo contiene el punto
      const candidates = [];
      for (const b of centers) {
        const d = distMeters(lat, lng, b.lat, b.lng);
        if (d <= R) candidates.push({ bufferId: b.bufferId, alias: b.alias, distM: d });
      }
      if (!candidates.length) return;

      // Orden: menor distancia, luego alias menor, luego bufferId
      candidates.sort((x, y) =>
        (x.distM - y.distM) ||
        (aliasNum(x.alias) - aliasNum(y.alias)) ||
        String(x.bufferId).localeCompare(String(y.bufferId))
      );

      const best = candidates[0];
      const casos = candidates.length;

      // Si este AMIE ya estaba, mantener la mejor asignación (por distancia / alias)
      const prev = assignmentByAmie.get(amie);
      if (!prev) {
        assignmentByAmie.set(amie, { bufferId: best.bufferId, distM: best.distM, casos });
        return;
      }
      const prevAlias = aliasById.get(prev.bufferId);
      const bestAlias = aliasById.get(best.bufferId);
      const better =
        (best.distM < prev.distM) ||
        (best.distM === prev.distM && (aliasNum(bestAlias) < aliasNum(prevAlias)));

      if (better) assignmentByAmie.set(amie, { bufferId: best.bufferId, distM: best.distM, casos });
    });

    // Agrupar por bufferId con Set de AMIE únicos
    const bufToAmies = new Map();   // bufferId -> Set<AMIE>
    const bufToCasos = new Map();   // bufferId -> Map<AMIE, casos>
    assignmentByAmie.forEach((v, amie) => {
      if (!v?.bufferId) return;
      if (!bufToAmies.has(v.bufferId)) bufToAmies.set(v.bufferId, new Set());
      bufToAmies.get(v.bufferId).add(amie);

      if (!bufToCasos.has(v.bufferId)) bufToCasos.set(v.bufferId, new Map());
      bufToCasos.get(v.bufferId).set(amie, Number(v.casos || 1));
    });

    // Ordenar buffers por alias numérico
    const sortedBuffers = list.slice().sort((a, b) =>
      (aliasNum(a.alias) - aliasNum(b.alias)) || String(a.alias).localeCompare(String(b.alias))
    );

    // Salida VERTICAL (solo buffers con satélites)
    for (const ob of sortedBuffers) {
      const alias = String(ob.alias || ob.bufferId);
      const set = bufToAmies.get(ob.bufferId);
      if (!set || set.size === 0) continue;

      const amies = Array.from(set).sort();
      const total = amies.length;
      const casosMap = bufToCasos.get(ob.bufferId) || new Map();

      for (const amie of amies) {
        rows.push({
          BUFFER_ALIAS: alias,
          SATELITES_FISCALES: amie,
          TOTAL_SATELITES: total,
          CASOS: Number(casosMap.get(amie) || 1)
        });
      }
    }

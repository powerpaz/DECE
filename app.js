...
function toggleEditMode() {
  ...
  console.log("🔧 Toggle Edit Mode:", editMode); // Debug
  ...
  if (editMode) {
    ...
    showNotification("✏️ Modo edición: Arrastra los buffers con el mouse", "info");

    // En edición: permitir agarrar el buffer aunque haya puntos encima
    setPointLayersInteractive(false);
    ...
    // Restaurar interacción de puntos
    setPointLayersInteractive(true);
    
    // Desactivar dragging
    editableBuffers.forEach(data => {
      disableDragging(data.circle, data);
    });
    customBuffers.forEach(buffer => {
      disableDragging(buffer.circle, buffer);
    });
  }
}
...
function toggleAddMode() {
  ...
  console.log("➕ Toggle Add Mode:", addMode); // Debug
  ...
  if (addMode) {
    btn?.classList.add("active");
    showNotification("➕ Modo añadir: Haz click en el mapa para crear un buffer", "info");
    ...
    // Crear y asignar nuevo listener
    mapClickListener = onMapClickForAdd;
...
function toggleDeleteMode() {
  ...
  console.log("🗑️ Toggle Delete Mode:", deleteMode); // Debug
  ...
  if (deleteMode) {
    btn?.classList.add("active");
    showNotification("🗑️ Modo eliminar: click en un buffer para borrarlo (también sirve SUPR/DELETE)", "info");

    // En eliminar: click debe caer en el buffer, no en núcleos/satélites
    setPointLayersInteractive(false);
    
    // Agregar listener de teclado
    if (!_deleteKeyListenerBound) {
      document.addEventListener('keydown', handleDeleteKey);
      _deleteKeyListenerBound = true;
    }
  } else {
    btn?.classList.remove("active");
    selectedDeleteTarget = null; // Limpiar selección
    ...
    if (_deleteKeyListenerBound) {
      ...
      _deleteKeyListenerBound = false;
    }
    
    // Restaurar estilos de todos los buffers
    editableBuffers.forEach((data) => {
      data.circle.setStyle({ color: '#58a6ff', weight: 2, opacity: 0.6 });
    });
    customBuffers.forEach(buffer => {
      buffer.circle.setStyle({ color: '#a371f7', weight: 2, opacity: 0.7 });
    });
  }
}

// Nueva función para manejar teclas Delete/Supr
function handleDeleteKey(e) {
  if (!deleteMode) return;
  
  // Detectar teclas Delete (46) o Supr (8 en algunos teclados)
  if (e.key === 'Delete' || e.keyCode === 46 || e.key === 'Backspace' || e.keyCode === 8) {
    e.preventDefault();
    
    if (selectedDeleteTarget) {
      // Hay un buffer seleccionado, eliminarlo
      deleteSelectedBuffer();
    } else {
      showNotification("⚠️ Primero haz click en un buffer para seleccionarlo", "warning");
    }
  }
}
...
// ==================== DRAGGING SYSTEM - FIXED (EDIT MODE) ====================
// Objetivo: al activar "Editar", poder arrastrar CUALQUIER buffer sin que el mapa se panee.
// Nota: Leaflet envía eventos "mousedown/touchstart" a layers vectoriales (SVG/Canvas) si son interactivos.

function makeBufferDraggable(circle, data, isCustom) {
  // Alias retrocompatible (en el código aún se llama desde algunos lugares)
  return enableDragging(circle, data, isCustom);
}

// Helper: detener de verdad el drag del mapa capturando el DOM event original
function stopMapInteractionEvent(leafletEvt) {
  const oe = leafletEvt?.originalEvent || leafletEvt;
  if (!oe) return;
  try { L.DomEvent.stopPropagation(oe); } catch (_) {}
  try { L.DomEvent.preventDefault(oe); } catch (_) {}
}


// Controla si los puntos (núcleos y satélites) capturan clicks/drag.
// En modo editar/eliminar los desactivamos para que el evento "pase" al buffer (círculo) debajo.
function setPointLayersInteractive(isInteractive) {
  const groups = [layers?.nucleos, layers?.satellites];
  groups.forEach(g => {
    ...
    g.eachLayer(layer => {
      ...
      // Si es SVG, ajustar pointer-events para asegurar passthrough.
      if (layer._path && layer._path.style) {
        layer._path.style.pointerEvents = isInteractive ? 'auto' : 'none';
      }
      // Redibujar (importante con renderer canvas)
...
function enableDragging(circle, data, isCustom) {
  ...
  // Asegurar que el círculo reciba eventos
  if (circle?.options) circle.options.interactive = true;

  // Evitar múltiples inicializaciones
  if (circle._draggingEnabled && circle._dragHandlers) {
    // Re-activar (por si venía de un disable incompleto)
    circle._draggingEnabled = true;
    return;
  }
  ...
  const onDown = function (e) {
    ...
    // Clave: parar el DOM event ORIGINAL para que el mapa NO inicie pan
    stopMapInteractionEvent(e);

    isDragging = true;
    startLatLng = circle.getLatLng();
    data.isDragging = true;

    // Deshabilitar pan del mapa mientras arrastramos el buffer
    if (map?.dragging) map.dragging.disable();

    // Escuchar movimientos SOLO durante el drag (más estable y menos ruido)
    map.on('mousemove touchmove', onMove);
    map.on('mouseup touchend touchcancel', onUp);

    L.DomUtil.addClass(map._container, 'leaflet-grab');
    L.DomUtil.addClass(map._container, 'leaflet-dragging');
  };

  const onMove = function (e) {
    if (!isDragging || !editMode) return;
    // Mover el círculo al puntero
    if (e?.latlng) circle.setLatLng(e.latlng);
  };
  ...
  const onUp = function (e) {
    ...
    // Si se movió, persistir
    if (startLatLng && (startLatLng.lat !== newPos.lat || startLatLng.lng !== newPos.lng)) {
      data.currentPos = { lat: newPos.lat, lng: newPos.lng };

      if (!isCustom) {
        data.adjusted = true;
        if (circle._adjusted !== undefined) circle._adjusted = true;
      }
  ...
  // Registrar handlers
  circle.on('mousedown touchstart', onDown);

  circle._dragHandlers = { onDown, onMove, onUp };
  circle._draggingEnabled = true;
}

function disableDragging(circle, data) {
  if (!circle?._dragHandlers) return;

  // Remover listener principal
  circle.off('mousedown touchstart', circle._dragHandlers.onDown);

  // Por si quedo a medias el drag
  map.off('mousemove touchmove', circle._dragHandlers.onMove);
  map.off('mouseup touchend touchcancel', circle._dragHandlers.onUp);

  delete circle._dragHandlers;
  circle._draggingEnabled = false;
  if (data) data.isDragging = false;

  // Asegurar que el mapa vuelva a panear
  if (map?.dragging) map.dragging.enable();
}

// ==================== DELETE HELPERS ====================

function deleteBufferTarget(target) {
  selectedDeleteTarget = target;
  deleteSelectedBuffer();
}
...
function deleteSelectedBuffer() {
  ...
  selectedDeleteTarget = null; // limpiar antes de eliminar

  try {
    if (target.isCustom) {
      // Eliminar buffer personalizado
      const index = customBuffers.findIndex(b => b.id === target.id);
      if (index >= 0) {
        const buf = customBuffers[index];
        const circle = buf?.circle;
        if (circle) {
          // Quitar handlers de drag si existieran
          disableDragging(circle, buf);
          layers.buffers?.removeLayer(circle);
          map.removeLayer(circle);
        }
        customBuffers.splice(index, 1);
        markAsChanged();
        showNotification("🗑️ Buffer personalizado eliminado", "success");
        updateCoverageOverlay();
        regenerateAnimations();
        updateCoverageAnalysis();
      } else {
        showNotification("⚠️ No se encontró el buffer para eliminar", "warning");
      }
      return;
    }

    // Eliminar buffer editable (original)
    const keysToTry = [target.ni, String(target.ni)];
    const asNum = Number(target.ni);
    if (!Number.isNaN(asNum)) keysToTry.push(asNum);

    let keyFound = null;
    let bufferData = null;

    for (const k of keysToTry) {
      if (editableBuffers.has(k)) {
        keyFound = k;
        bufferData = editableBuffers.get(k);
        break;
      }
    }

    if (!bufferData) {
      // Fallback: buscar por equivalencia string
      for (const [k, v] of editableBuffers.entries()) {
        if (String(k) === String(target.ni)) {
          keyFound = k;
          bufferData = v;
          break;
        }
    ...
    if (bufferData && bufferData.circle) {
      const circle = bufferData.circle;
      disableDragging(circle, bufferData);
      layers.buffers?.removeLayer(circle);
      map.removeLayer(circle);
      if (keyFound !== null) editableBuffers.delete(keyFound);
    ...
    console.error("❌ Error al eliminar buffer:", err);
    showNotification("❌ No se pudo eliminar el buffer (ver consola)", "error");
  }
}
...
function onMapClickForAdd(e) {
  ...
  circle.on('click', (e) => {
    ...
    if (deleteMode) {
      // En modo eliminar: click = eliminar (sin teclas)
      deleteBufferTarget({ circle, id: newBuffer.id, isCustom: true });
      return;
    }
...
function drawBuffersEditable(nucleos, selected, nucleoStats) {
  ...
  selected.forEach(ni => {
    ...
    circle.on('click', (e) => {
      ...
      if (deleteMode) {
        // En modo eliminar: click = eliminar (sin teclas)
        deleteBufferTarget({ circle, ni, isCustom: false });
        return;
      }
    ...
    // 🔧 FIX: Si el modo edición ya está activo, hacer este buffer arrastrable inmediatamente
    if (editMode) {
...
function restoreCustomBuffer(saved) {
  ...
  circle.on('click', (e) => {
    ...
    if (deleteMode) {
      // En modo eliminar: click = eliminar (sin teclas)
      deleteBufferTarget({ circle, id: buffer.id, isCustom: true });
      return;
    }
  ...
  // 🔧 FIX: Siempre activar dragging si editMode está activo
  if (editMode) {
...
function optimizeBuffers() {
  if (!globalData) {
    showNotification("⚠️ No hay datos cargados", "warning");
    return;
  }
  
  if (!confirm('¿Optimizar cobertura automáticamente?\n\n1. Eliminará buffers vacíos\n2. Sugerirá posiciones óptimas\n3. Mejorará cobertura general')) {
    return;
  }
  
  showNotification("🔄 Optimizando cobertura...", "info");
  
  // Paso 1: Analizar y eliminar buffers vacíos
  analyzeEmptyBuffers();
  let optimizationSteps = [];
  
  if (emptyBuffers.length > 0) {
    optimizationSteps.push(`- Encontrados ${emptyBuffers.length} buffers vacíos`);
    
    emptyBuffers.forEach(buffer => {
      if (buffer.type === 'custom') {
        const customIndex = customBuffers.findIndex(b => b.id === buffer.id);
        if (customIndex >= 0) {
          map.removeLayer(customBuffers[customIndex].circle);
          customBuffers.splice(customIndex, 1);
        }
      } else {
        const bufferData = editableBuffers.get(buffer.ni);
    ...
    optimizationSteps.push(`- Eliminados ${emptyBuffers.length} buffers vacíos`);
    emptyBuffers = [];
  }
  
  // Paso 2: Identificar zonas sin cobertura
  identifyUncoveredZones();
  
  if (uncoveredPoints.length > 0) {
    optimizationSteps.push(`- Encontrados ${uncoveredPoints.length} puntos sin cobertura`);
    
    // Agrupar puntos cercanos para sugerir ubicaciones de buffers
    const clusters = clusterUncoveredPoints(uncoveredPoints);
    optimizationSteps.push(`- Identificados ${clusters.length} grupos de puntos`);
    
    // Sugerir nuevos buffers en centroides de clusters
    let buffersAdded = 0;
    clusters.forEach((cluster, idx) => {
      if (customBuffers.length + editableBuffers.size < MAX_BUFFERS) {
        const centroid = calculateCentroid(cluster.points);
        customBufferCounter++;
    ...
    renderer: canvasRenderer
        ...
        const newBuffer = {
          id: `optimized_${customBufferCounter}`,
          circle,
          lat: centroid.lat,
          lng: centroid.lng,
          originalPos: { lat: centroid.lat, lng: centroid.lng },
          currentPos: { lat: centroid.lat, lng: centroid.lng },
          isCustom: true,
          isDragging: false,
          name: `Buffer Optimizado ${customBufferCounter}`,
          adjusted: false
        };
      ...
      return;
        ...
        if (editMode) makeBufferDraggable(circle, newBuffer, true);
      }
    });
    
    if (buffersAdded > 0) {
      optimizationSteps.push(`- Añadidos ${buffersAdded} buffers optimizados (verde)`);
    }
  }
  ...
  // Mostrar resumen
  const summary = `✅ Optimización completada:\n\n${optimizationSteps.join('\n')}\n\n💡 Los nuevos buffers son verdes.\n📍 Puedes moverlos en modo Edición.\n💾 No olvides Guardar los cambios.`;
...
function clusterUncoveredPoints(points) {
  const clusters = [];
  const processed = new Set();
  const CLUSTER_DISTANCE = BUFFER_RADIUS_M * 1.5; // 11.25 km
  
  points.forEach((point, idx) => {
    if (processed.has(idx)) return;
    
    const cluster = { points: [point], indices: [idx] };
    processed.add(idx);
    ...
    points.forEach((otherPoint, otherIdx) => {
      ...
      const dist = haversineMeters(point.lat, point.lng, otherPoint.lat, otherPoint.lng);
    ...
    // Solo clusters con al menos 3 puntos
...
function diagnosticBuffersDragging() {
  ...
  editableBuffers.forEach((data, ni) => {
    if (data.circle?._draggingEnabled) {
      ...
      nonDraggableCount++;
      console.log("  ⚠️ Buffer NO arrastrable:", ni, data.nucleo?.name);
    }
  });
  
  customBuffers.forEach((buffer) => {
    if (buffer.circle?._draggingEnabled) {
      draggableCount++;
    } else {
      nonDraggableCount++;
      console.log("  ⚠️ Custom buffer NO arrastrable:", buffer.id, buffer.name);
    }
  });
  ...
  if (editMode && nonDraggableCount > 0) {
    console.error("❌ ERROR: Hay buffers NO arrastrables en modo edición!");
  }
}

// Hacer la función accesible desde consola
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ===== CONFIGURAR BOTONES PRINCIPALES - FIXED =====
  ...
  if (btnAdd) {
    console.log("✅ Add button found, adding event listener");
  ...
  if (btnDelete) {
    ...
    btnDelete.addEventListener("click", (e) => {
      console.log("🗑️ Delete button clicked!"); // Debug
      e.preventDefault();
      e.stopPropagation();
      toggleDeleteMode();
    });
  ...
  document.getElementById("btnOptimizar")?.addEventListener("click", optimizeBuffers);
...
console.log("📦 DECE App v7.1 - Buttons Fixed - Loaded successfully!");
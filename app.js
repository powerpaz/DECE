 ...
 * Correcciones:
 * - Parse CSV robusto (delimiter ';' + BOM)
 * - Detección de índices por encabezados (sin depender de posición)
 * - Render Canvas + capas no interactivas (buffers/conexiones)
 * - Procesamiento por lotes (buffers, satélites, núcleos) para evitar "freeze"
 * - Indexación espacial simple (grid) para acelerar búsqueda de núcleo cercano
 *************************************************/
...
// Canvas renderer (mejora performance con muchos vectores)
...
function loadCSV() {
  ...
  Papa.parse('DECE_CRUCE_X_Y_NUC_SAT.csv', {
    download: true,
    delimiter: ';',           // el archivo real usa ;
    skipEmptyLines: 'greedy',
    worker: true,
    beforeFirstChunk: (chunk) => chunk.replace(/^\uFEFF/, ''), // BOM
    complete: (results) => {
      ...
      if (!data.length) {
        if (overlayText) overlayText.textContent = 'No se encontraron registros válidos (revisa columnas lat/long y COD_GDECE).';
        console.error('No valid data after mapping.');
        return;
      }

      // Opcional: encuadrar Ecuador/extent de datos (una vez)
...
// Determina índices por encabezados (tolerante a mayúsculas/BOM/espacios)
function resolveColumnIndexes(headerRow) {
  ...
  const idx = {
    ...
    cod: findOne(['cod_gdece', 'COD_GDECE', 'cod gdece']),
    name: findOne(['nombre_institución', 'nombre_institucion', 'nombre de la institución', 'nombre institucion', 'Nombre_Institución', 'nombre']),
    dist: findOne(['distrito', 'DISTRITO']),
    zone: findOne(['zona', 'Zona']),
    students: findOne(['total estudiantes', 'Total Estudiantes', 'total_estudiantes', 'TOTAL ESTUDIANTES']),
    profs: findOne(['po_profdece', 'PO_ProfDECE', 'profdece', 'profesionales', 'nro profesionales'])
  };
  ...
  // name/dist/students/profs pueden faltar; no son fatales, pero afectan popups/estadísticas
...
function processData(data) {
  ...
  // 1) Dibujar buffers por lotes (para no congelar)
  if (overlayText) overlayText.textContent = `Dibujando buffers… 0 / ${nucleos.length.toLocaleString()}`;
  drawBuffersInBatches(nucleos, 0, 400, () => {
    // 2) Procesar satélites por lotes
    if (overlayText) overlayText.textContent = `Asignando satélites… 0 / ${satellites.length.toLocaleString()}`;
    assignSatellitesInBatches(satellites, nucleos, grid, nucleoStats, 0, 250, (satCovered) => {
      // 3) Dibujar núcleos por lotes
      if (overlayText) overlayText.textContent = `Dibujando núcleos… 0 / ${nucleos.length.toLocaleString()}`;
      drawNucleosInBatches(nucleos, nucleoStats, 0, 400, () => {
        // UI final
...
function drawBuffersInBatches(nucleos, start, batchSize, done) {
  ...
  if (end < nucleos.length) requestAnimationFrame(() => drawBuffersInBatches(nucleos, end, batchSize, done));
  else done();
}

function assignSatellitesInBatches(satellites, nucleos, grid, nucleoStats, start, batchSize, done) {
  const overlayText = document.querySelector('#loadingOverlay .loading-text');

  let satellitesCovered = 0;
  // Reusar contador acumulado entre batches usando closure:
  ...
  if (end < satellites.length) {
    requestAnimationFrame(() => assignSatellitesInBatches(satellites, nucleos, grid, nucleoStats, end, batchSize, done));
  } else {
    satellitesCovered = assignSatellitesInBatches._covered || 0;
    assignSatellitesInBatches._covered = null;
    done(satellitesCovered);
  }
}

function drawNucleosInBatches(nucleos, nucleoStats, start, batchSize, done) {
  const overlayText = document.querySelector('#loadingOverlay .loading-text');
  ...
  if (end < nucleos.length) requestAnimationFrame(() => drawNucleosInBatches(nucleos, nucleoStats, end, batchSize, done));
  else done();
}

...
function exportToCSV() {
  ...
  console.log("[DATA] Exportando reporte (normal + forzado)...");

  const { nucleos, satellites, satCandidates, selected } = globalData;

  // Recalcular asignaciones forzadas y stats (por si hubo cambios)
  assignOrphanSatellites(satellites, nucleos, satCandidates, selected);
  identifyOrphanNucleos(nucleos, satellites, satCandidates, selected);
  refreshCoverageStats(nucleos, satellites, satCandidates, selected);

  // Header (incluye columnas solicitadas + extras útiles al final)
  let csv = "TIPO,NI,NOMBRE,LAT,LNG,BUFFER_LAT,BUFFER_LNG,DISTANCIA_M,FORZADO,DISTRITO,ESTUDIANTES\n";

  // ---- Satélites ----
  satellites.forEach((sat, si) => {
    const assign = getAssignmentForSatellite(si, nucleos, satCandidates, selected);
    const bufferLat = assign ? assign.bufferLat : "";
    const bufferLng = assign ? assign.bufferLng : "";
    const dist = assign ? Math.round(assign.distance) : "";
    const forzado = assign ? (assign.forced ? "SI" : "NO") : "NO";
  ...
  // ---- Núcleos (buffers seleccionados) ----
  selected.forEach((ni) => {
    const n = nucleos[ni];
    const center = getBufferCenterForNucleo(ni, nucleos);

    let bufferLat = center.lat, bufferLng = center.lng;
    let dist = 0, forzado = "NO";

    if (orphanNucleos.has(ni)) {
      const info = orphanNucleos.get(ni);
      const targetCenter = getBufferCenterForNucleo(info.mergeInto, nucleos);
      bufferLat = targetCenter.lat;
      bufferLng = targetCenter.lng;
      dist = Math.round(info.distance);
      forzado = "SI";
    }

    const row = [
      "nucleo",
      csvEscape(n.amie || `N-${ni}`),
      csvEscape(n.name || ""),
      n.lat,
      n.lng,
      bufferLat,
  ...
  csv += `Núcleos huérfanos (seleccionados),${orphanNucleos.size}\n`;
...
}"`,buffer.isCustom?'Personalizado':'Original',buffer.centerLat,buffer.centerLng,inst.amie,`"${inst.name}"`,inst.typeName,inst.codGDECE,inst.lat,inst.lng,inst.distanceMeters,inst.distanceKm,inst.students,`"${inst.distrito}"`].join(','))));
...
// ==================== COBERTURA TOTAL (ASIGNACIÓN FORZADA) ====================

function getBufferCenterForNucleo(ni, nucleos) {
  const data = editableBuffers.get(ni);
  if (data?.circle) {
    const pos = data.circle.getLatLng();
...
function findClosestSelectedNucleo(lat, lng, nucleos, selected, excludeNi = null) {
  let bestNi = null, bestDist = Infinity, bestCenter = null;
  selected.forEach(ni => {
    if (excludeNi !== null && ni === excludeNi) return;
    const center = getBufferCenterForNucleo(ni, nucleos);
    const dist = haversineMeters(lat, lng, center.lat, center.lng);
    if (dist < bestDist) { bestDist = dist; bestNi = ni; bestCenter = center; }
  });
  return bestNi === null ? null : { ni: bestNi, distance: bestDist, center: bestCenter };
}

// Asigna satélites sueltos al núcleo seleccionado más cercano (sin límite de distancia)
function assignOrphanSatellites(satellites, nucleos, satCandidates, selected) {
  forcedAssignments = new Map();
  if (!selected || selected.size === 0) return forcedAssignments;

  satellites.forEach((sat, si) => {
    // ¿Tiene cobertura normal dentro del radio?
    const coveredNormally = satCandidates?.[si]?.some(c => selected.has(c.ni) && c.dist <= BUFFER_RADIUS_M);
    if (coveredNormally) return;

    // Forzar a núcleo más cercano (aunque esté fuera del radio)
    const closest = findClosestSelectedNucleo(sat.lat, sat.lng, nucleos, selected);
    if (closest) forcedAssignments.set(si, { ni: closest.ni, distance: closest.distance });
  });

  return forcedAssignments;
}

// Identifica núcleos (seleccionados) sin satélites asignados y sugiere fusión con el más cercano
function identifyOrphanNucleos(nucleos, satellites, satCandidates, selected) {
  orphanNucleos = new Map();
  if (!selected || selected.size === 0) return orphanNucleos;

  const assignedCount = new Map();
  selected.forEach(ni => assignedCount.set(ni, 0));

  satellites.forEach((sat, si) => {
    // Primero: asignación normal (en radio)
    let bestNi = null;
    const cands = satCandidates?.[si] || [];
    for (let k = 0; k < cands.length; k++) {
      const c = cands[k];
      if (selected.has(c.ni) && c.dist <= BUFFER_RADIUS_M) { bestNi = c.ni; break; }
    }

    // Segundo: asignación forzada (si aplica)
    if (bestNi === null && forcedAssignments.has(si)) bestNi = forcedAssignments.get(si).ni;

    if (bestNi !== null && assignedCount.has(bestNi)) assignedCount.set(bestNi, assignedCount.get(bestNi) + 1);
  });

  selected.forEach(ni => {
    if ((assignedCount.get(ni) || 0) > 0) return;
    const center = getBufferCenterForNucleo(ni, nucleos);
    const closest = findClosestSelectedNucleo(center.lat, center.lng, nucleos, selected, ni);
    if (closest) orphanNucleos.set(ni, { mergeInto: closest.ni, distance: closest.distance });
  });

  return orphanNucleos;
}

function refreshCoverageStats(nucleos, satellites, satCandidates, selected) {
  coverageStats.totalSatellites = satellites.length;
  let coveredNormal = 0;

  satellites.forEach((s, si) => {
    const ok = satCandidates?.[si]?.some(c => selected.has(c.ni) && c.dist <= BUFFER_RADIUS_M);
    if (ok) coveredNormal++;
  });

  const coveredForced = forcedAssignments.size;
  const orphanCount = Math.max(0, satellites.length - coveredNormal - coveredForced);

  coverageStats.coveredNormal = coveredNormal;
  coverageStats.coveredForced = coveredForced;
  coverageStats.orphanCount = orphanCount;

  coverageStats.normalCoveragePercent = satellites.length ? ((coveredNormal / satellites.length) * 100).toFixed(1) : "0.0";
  coverageStats.totalCoveragePercent = satellites.length ? (((coveredNormal + coveredForced) / satellites.length) * 100).toFixed(1) : "0.0";

  return coverageStats;
}

function getAssignmentForSatellite(si, nucleos, satCandidates, selected) {
  // normal
  let bestNi = null, bestDist = Infinity, forced = false;
  const cands = satCandidates?.[si] || [];
  for (let k = 0; k < cands.length; k++) {
    const c = cands[k];
    if (selected.has(c.ni) && c.dist <= BUFFER_RADIUS_M && c.dist < bestDist) { bestNi = c.ni; bestDist = c.dist; }
  }
  if (bestNi === null && forcedAssignments.has(si)) {
    const f = forcedAssignments.get(si);
    bestNi = f.ni; bestDist = f.distance; forced = true;
  }
  if (bestNi === null) return null;
  const center = getBufferCenterForNucleo(bestNi, nucleos);
  return { ni: bestNi, distance: bestDist, forced, bufferLat: center.lat, bufferLng: center.lng };
}
  ...
  fetch("DECE_CRUCE_X_Y_NUC_SAT.csv", { cache: "no-store" })
    ...
    .catch(err => {
      ...
      if (location.protocol === "file:") {
        setText("Error cargando CSV", "Sugerencia: abre con un servidor local (VSCode Live Server o: python -m http.server)");
      } else {
        setText("Error cargando CSV", msg);
      }
    });
...
function resolveColumnIndexes(headerRow) {
  ...
  const find = (candidates) => {
    for (let c of candidates) {
      const cc = norm(c);
      const idx = header.findIndex(h => h === cc || h.includes(cc));
      if (idx >= 0) return idx;
    }
    return -1;
  };

  // Soporta CSV original (con CODE 2-5) y CSV crudo Mineduc (COORD_DECE 0-3)
  const codeIdx = find(["code", "codigo", "código", "tipo", "type", "categoria", "categoría", "coord_dece", "coord dece", "coorddece", "coorddece", "coor_dece"]);
  const codeHeader = codeIdx >= 0 ? header[codeIdx] : "";
  const codeIsCoordDece = codeHeader.includes("coord_dece") || codeHeader.includes("coord dece") || codeHeader.includes("coorddece") || codeHeader.includes("coor_dece");

  const idx = {
    lat: find(["lat", "latitud", "latitude"]),
    lon: find(["lon", "lng", "long", "longitud", "longitude"]),
    code: codeIdx,
    codeIsCoordDece,
    name: find(["nombre", "name", "institucion", "institución", "nombre_institución", "nombre institución", "nombre_institucion", "nombreinstitución"]),
    dist: find(["distrito", "district"]),
    students: find(["total estudiantes", "total_estudiantes", "totalestudiantes", "estudiantes", "students"]),
    amie: find(["amie", "ni", "id", "codigo amie", "código amie"])
  };

  const issues = [];
  if (idx.lat < 0 || idx.lon < 0) issues.push("No se detectaron columnas de lat/lon");
  if (idx.code < 0) issues.push("No se detectó columna de tipo (CODE o COORD_DECE)");
  return { idx, issues };
}
...
function mapRowsToData(rows, idx) {
  ...
  for (let i = 1; i < (rows || []).length; i++) {
    ...
    let codeVal = parseInt(String(r[idx.code] || "").trim(), 10);
    if (!Number.isFinite(codeVal)) continue;

    // Si viene como COORD_DECE (0-3), mapear a CODE (2-5)
    if (idx.codeIsCoordDece) codeVal = codeVal + 2;
...
function processData(data) {
  console.log("[DEBUG] processData iniciado con", data.length, "registros");
  ...
  // Dibujar primero buffers (para recuperar posiciones guardadas)
  drawNucleos(nucleos, result.selected);
  drawBuffersEditable(nucleos, result.selected, nucleoStats);

  // Completar cobertura SIN mover buffers: asignación forzada + núcleos huérfanos
  assignOrphanSatellites(satellites, nucleos, satCandidates, result.selected);
  identifyOrphanNucleos(nucleos, satellites, satCandidates, result.selected);
  refreshCoverageStats(nucleos, satellites, satCandidates, result.selected);
  ...
  console.log("[OK] globalData guardado:", {
    ...
    forcedAssignments: forcedAssignments.size,
    orphanNucleos: orphanNucleos.size
  });
...
function drawSatellites(satellites, satCandidates, selected) {
  satellites.forEach((s, si) => {
    const assign = getAssignmentForSatellite(si, globalData.nucleos, satCandidates, selected);
    const isCovered = !!assign;
    const isForced = !!assign?.forced;

    const marker = L.circleMarker([s.lat, s.lng], {
      radius: 4,
      color: isCovered ? (isForced ? "#ffb86c" : "#2ea043") : "#f85149",
      fillColor: isCovered ? (isForced ? "#ffb86c" : "#2ea043") : "#f85149",
      weight: 1,
      opacity: 1,
      fillOpacity: 0.8,
    ...
    marker.bindPopup(createSatellitePopup(s, isCovered ? assign.distance : null, isForced));
...
function createSatellitePopup(s, distMetersOrNull, forced = false) {
  const assigned = distMetersOrNull !== null;
  const mode = !assigned ? "Sin asignar" : (forced ? "Forzada" : "En radio");
  return `
    <div class="popup-title">📍 Satélite</div>
    <div class="popup-content">
      <div class="popup-row"><span class="popup-label">NI:</span><span class="popup-value">${escapeHTML(s.amie || "")}</span></div>
      <div class="popup-row"><span class="popup-label">Nombre:</span><span class="popup-value">${escapeHTML(s.name || "")}</span></div>
      <div class="popup-row"><span class="popup-label">Distrito:</span><span class="popup-value">${escapeHTML(s.dist || "")}</span></div>
      <div class="popup-row"><span class="popup-label">Asignación:</span><span class="popup-value">${mode}</span></div>
      ${assigned ? `<div class="popup-row"><span class="popup-label">Distancia:</span><span class="popup-value">${(distMetersOrNull/1000).toFixed(2)} km</span></div>` : ""}
      <div class="popup-row"><span class="popup-label">Estudiantes:</span><span class="popup-value">${(s.students || 0).toLocaleString()}</span></div>
    </div>
  `;
}
...
function updateStatistics(stats) {
  ...
  // Nuevas métricas (si existen en el HTML)
  set("coveredNormal", stats.coveredNormal);
  set("coveredForced", stats.coveredForced);
  set("coverageNormalPercent", stats.normalCoveragePercent + "%");
  set("orphanNucleosCount", stats.orphanNucleosCount);
...
function computeStatistics(nucleos, satellites, satCandidates, selected, nucleoStats) {
  // Asegurar que existan asignaciones forzadas y stats actualizados
  if (!forcedAssignments || forcedAssignments.size === 0) {
    assignOrphanSatellites(satellites, nucleos, satCandidates, selected);
  }
  identifyOrphanNucleos(nucleos, satellites, satCandidates, selected);
  refreshCoverageStats(nucleos, satellites, satCandidates, selected);

  // Sumar estudiantes asignados (normal + forzado)
  let totalStudents = 0;
  satellites.forEach((s, si) => {
    const assign = getAssignmentForSatellite(si, nucleos, satCandidates, selected);
    if (assign) totalStudents += s.students || 0;
  });

  return {
    totalNucleos: nucleos.length,
    totalSatellites: satellites.length,
    coveragePercent: coverageStats.totalCoveragePercent,
    normalCoveragePercent: coverageStats.normalCoveragePercent,
    totalStudents,
    nucleosActivos: selected.size,
    sinCobertura: coverageStats.orphanCount,
    coveredNormal: coverageStats.coveredNormal,
    coveredForced: coverageStats.coveredForced,
    orphanNucleosCount: orphanNucleos.size
  };
}

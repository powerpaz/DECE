...
const MAX_FORCED_ASSIGNMENT_M = 7000; // Máxima distancia para asignaciones/ fusiones de huérfanos
...
let orphanAnalysis = {
  ...
  unservedSatellitesInfo: new Map(), // si -> {closestNi, distance}
  orphanSatellites: new Set(), // satélites sin atención dentro del umbral
  orphanNucleos: new Set(),
  nucleoFusionCandidates: new Map(), // ni(orphan) -> {closestNi, distance}
...
function analyzeOrphans() {
  ...
  orphanAnalysis.unservedSatellitesInfo.clear();
  orphanAnalysis.orphanSatellites.clear();
  orphanAnalysis.orphanNucleos.clear();
  orphanAnalysis.nucleoFusionCandidates.clear();
  ...
  satellites.forEach((sat, si) => {
    ...
    if (covered) {
      ...
      if (closestNi !== null && minDist <= MAX_FORCED_ASSIGNMENT_M) {
        orphanAnalysis.forcedAssignments.set(si, { ni: closestNi, distance: minDist });
        forcedCovered++;
      } else {
        // Demasiado lejos (ej. Galápagos vs continente) o sin núcleos seleccionados
        orphanAnalysis.orphanSatellites.add(si);
        if (closestNi !== null) {
          orphanAnalysis.unservedSatellitesInfo.set(si, { closestNi, distance: minDist });
        }
      }
    }
  });
  ...
  selected.forEach(ni => {
    if (!hasAnySatellite[ni]) {
      orphanAnalysis.orphanNucleos.add(ni);
      // Propuesta de fusión: núcleo seleccionado más cercano (solo si <= umbral)
      let minD = Infinity;
      let closest = null;
      const nucA = nucleos[ni];
      if (!nucA) return;
      const bufA = editableBuffers.get(ni);
      const latA = bufA?.currentPos?.lat || nucA.lat;
      const lngA = bufA?.currentPos?.lng || nucA.lng;
      selected.forEach(otherNi => {
        if (otherNi === ni) return;
        const nucB = nucleos[otherNi];
        if (!nucB) return;
        const bufB = editableBuffers.get(otherNi);
        const latB = bufB?.currentPos?.lat || nucB.lat;
        const lngB = bufB?.currentPos?.lng || nucB.lng;
        const d = haversineMeters(latA, lngA, latB, lngB);
        if (d < minD) { minD = d; closest = otherNi; }
      });
      if (closest !== null && minD <= MAX_FORCED_ASSIGNMENT_M) {
        orphanAnalysis.nucleoFusionCandidates.set(ni, { closestNi: closest, distance: minD });
      }
    }
  });
...
function updateOrphanPanel() {
  ...
  panel.innerHTML = `
    ...
    <div class="orphan-stat danger">
      <div class="stat-label">Sin atención (&gt;7 km)</div>
      <div class="stat-value">${orphanAnalysis.orphanSatellites.size}</div>
      <div class="stat-sub">satélites huérfanos</div>
    </div>
...
function drawOrphanConnections() {
  ...
  // Resaltar satélites sin atención (ej. islas sin núcleo seleccionado)
  if (!layers.unservedSatellites) {
    layers.unservedSatellites = L.featureGroup().addTo(map);
  } else {
    layers.unservedSatellites.clearLayers();
  }

  orphanAnalysis.orphanSatellites.forEach((si) => {
    const sat = satellites[si];
    if (!sat) return;
    const info = orphanAnalysis.unservedSatellitesInfo.get(si);
    let detail = "Sin núcleo seleccionado a <= 7 km";
    if (info) {
      const nuc = nucleos[info.closestNi];
      const nucName = nuc ? nuc.name : `NI ${info.closestNi}`;
      detail = `Más cercano: ${nucName} (${(info.distance/1000).toFixed(2)} km)`;
    }
    ...
    m.bindPopup(`<b>Satélite sin atención</b><br>${sat.name}<br>${detail}`);
    layers.unservedSatellites.addLayer(m);
  });

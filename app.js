...
function recomputeOrphanBufferCoverage() {
  ...
  // ================================
  // Regla de CALIDAD (observación):
  // Un AMIE (satélite) debe pertenecer a UN solo Buffer B#.
  // En la data pueden existir registros duplicados del mismo AMIE (mismo o distinto punto).
  // Para evitar que aparezca en 2 o 3 buffers en la exportación, hacemos una asignación
  // determinística por AMIE: el buffer cuyo centro esté más cerca (entre los buffers que lo contienen).
  // Empate: menor número de alias (B2 < B4), luego bufferId.
  // ================================
  ...
  // 1) Centros reales de los buffers huérfanos (B#)
  const orphanCenters = [];
  orphanBuffersState.list.forEach(ob => {
    ...
    orphanCenters.push({
      bufferId,
      alias: String(ob.alias || bufferId),
      aliasN: aliasNum(ob.alias || bufferId),
      lat: center.lat,
      lng: center.lng
    });
  });
  if (!orphanCenters.length) return;

  // 2) Agrupar satélites elegibles por AMIE (para manejar duplicados)
  const satByAmie = new Map(); // amie -> [{si, lat, lng}]
  satellites.forEach((sat, si) => {
    if (!isFiscalInstitution(sat) || !isSatellite51to120(sat)) return;
    const amie = String(sat?.amie || '').trim();
    if (!amie) return;
    if (!satByAmie.has(amie)) satByAmie.set(amie, []);
    satByAmie.get(amie).push({ si, lat: sat.lat, lng: sat.lng });
  });

  // 3) Asignación única AMIE -> bufferId (determinística)
  const amieToOrphanBuffer = new Map();
  satByAmie.forEach((pts, amie) => {
    let best = null;
    let bestD = Infinity;

    for (const ob of orphanCenters) {
      // Distancia mínima del AMIE (si tiene duplicados) al centro del buffer
      let dMin = Infinity;
      for (const p of pts) {
        const d = haversineMeters(ob.lat, ob.lng, p.lat, p.lng);
        if (d < dMin) dMin = d;
      }
      if (dMin <= BUFFER_RADIUS_M) {
        if (
          dMin < bestD ||
          (dMin === bestD && best && (ob.aliasN < best.aliasN)) ||
          (dMin === bestD && best && (ob.aliasN === best.aliasN) && String(ob.bufferId) < String(best.bufferId))
        ) {
          best = ob;
          bestD = dMin;
        }
    ...
    if (best) amieToOrphanBuffer.set(amie, best.bufferId);
  });

  // 4) Poblar satInsideOrphan por índice de satélite (si) usando la asignación por AMIE
  satellites.forEach((sat, si) => {
    if (!isFiscalInstitution(sat) || !isSatellite51to120(sat)) return;
    const amie = String(sat?.amie || '').trim();
    const bid = amieToOrphanBuffer.get(amie);
    if (bid) orphanBufferCoverageState.satInsideOrphan.set(si, bid);
  });

  // 5) Luego calculamos cobertura por buffer (usando satInsideOrphan ya “limpio”)
  orphanBuffersState.list.forEach(ob => {
    const bufferId = ob.bufferId;
    ...
    // satélites asignados a ESTE buffer (7 km) — 1 AMIE -> 1 buffer
    const satsInBuffer = [];
    satellites.forEach((sat, si) => {
      if (!isFiscalInstitution(sat) || !isSatellite51to120(sat)) return;
      const bid = orphanBufferCoverageState.satInsideOrphan.get(si);
      if (bid !== bufferId) return;
...
async function tryLoadOfficialState(){
  ...
  try {
    ...
    // Para evitar que en tu PC “gane” un localStorage viejo (44.88%) o se mezcle con backup,
    // primero limpiamos el estado local de buffers y luego cargamos el oficial.
    try {
      localStorage.removeItem(STORAGE_KEY);
      localStorage.removeItem(BACKUP_KEY);
    } catch(_) {}

    // Marca global: el estado oficial debe tener prioridad sobre cualquier estado local.
    window.__DECE_FORCE_OFFICIAL__ = true;
  ...
  modal.innerHTML = `
    <div class="export-panel">
      ...
      <div class="export-content">
        <div class="export-summary" style="display:none">
...
function buildBuffersBRowsForExport() {
  // Export ESPEJO de satélites fiscales dentro de buffers con alias (B#),
  // garantizando:
  // 1) Un registro por (BUFFER_ALIAS, SATELITES_FISCALES)
  // 2) Un SATELITES_FISCALES no puede pertenecer a más de un BUFFER_ALIAS (si hay solape, se asigna al más cercano)
  try {
    const rawRows = window._rawRowsForExport || [];
    const R = Number(window.BUFFER_RADIUS_METERS || (typeof BUFFER_RADIUS_METERS !== "undefined" ? BUFFER_RADIUS_METERS : 7500));
    if (!Array.isArray(rawRows) || !rawRows.length) return [];

    // ===== 1) Construir lista de buffers "con alias" =====
    // Preferimos orphanBuffersState (si existe y está poblado). Si no, usamos customBuffers.
    let buffers = [];
    if (orphanBuffersState && Array.isArray(orphanBuffersState.list) && orphanBuffersState.list.length) {
      buffers = orphanBuffersState.list
        .map(ob => ({
          bufferId: ob.bufferId,
          alias: String(ob.alias || ob.bufferId),
          lat: Number(ob.centerLat),
          lng: Number(ob.centerLng)
        }))
        .filter(b => Number.isFinite(b.lat) && Number.isFinite(b.lng));
    } else {
      // Fallback: derivar alias B1..Bn desde customBuffers (orden estable N→S, O→E)
      const tmp = (customBuffers || [])
        .map(b => {
          const ll = b?.circle?.getLatLng?.();
          return ll ? ({ bufferId: b.id, lat: ll.lat, lng: ll.lng }) : null;
        })
        .filter(Boolean)
        .filter(b => Number.isFinite(b.lat) && Number.isFinite(b.lng));

      tmp.sort((a,b) => (b.lat - a.lat) || (a.lng - b.lng) || (String(a.bufferId).localeCompare(String(b.bufferId))));
      buffers = tmp.map((b, i) => ({ ...b, alias: `B${i+1}` }));
    }
    ...
    const distMeters = (lat1, lng1, lat2, lng2) => {
      if (window.L && L.latLng) return L.latLng(lat1, lng1).distanceTo(L.latLng(lat2, lng2));
    ...
    // ===== 2) Filtrar satélites fiscales (los que exportas hoy) =====
    const fiscalSats = rawRows.filter(r => {
      const amie = String(r?.amie || '').trim();
      if (!amie) return false;
      const isSat = isSatellite51to120(r);
      const isFis = (r?.ieFiscales === true) || isFiscalOnly(r?.sostenimiento);
      return isSat && isFis;
    });

    if (!fiscalSats.length) return [];

    // ===== 3) Asignación 1 satélite -> 1 buffer (si solape) =====
    // Construye candidatos por satélite, elige el más cercano.
    const chosen = new Map(); // AMIE -> { alias, bufferId, casos, bestDist }

    for (const s of fiscalSats) {
      const amie = String(s.amie || '').trim().toUpperCase();
      const lat = Number(s.lat), lng = Number(s.lng);
      if (!amie || !Number.isFinite(lat) || !Number.isFinite(lng)) continue;

      let best = null;
      let casos = 0;

      for (const b of buffers) {
        const d = distMeters(lat, lng, b.lat, b.lng);
        if (d <= R) {
          casos++;
          const cand = { alias: b.alias, bufferId: b.bufferId, d };
          if (!best) best = cand;
          else if (cand.d < best.d - 0.01) best = cand;
          else if (Math.abs(cand.d - best.d) <= 0.01) {
            const an = aliasNum(cand.alias), bn = aliasNum(best.alias);
            if (an < bn) best = cand;
            else if (an === bn && String(cand.bufferId) < String(best.bufferId)) best = cand;
          }
        }
      }
      ...
      // Si el AMIE aparece duplicado en el CSV, conserva el mejor (más cercano) y acumula CASOS como max.
      const prev = chosen.get(amie);
      if (!prev) chosen.set(amie, { alias: best.alias, bufferId: best.bufferId, casos, bestDist: best.d });
      else {
        const bestPrev = prev.bestDist;
        const takeNew = best.d < bestPrev - 0.01 ||
                        (Math.abs(best.d - bestPrev) <= 0.01 && (aliasNum(best.alias) < aliasNum(prev.alias)));
        chosen.set(amie, {
          alias: takeNew ? best.alias : prev.alias,
          bufferId: takeNew ? best.bufferId : prev.bufferId,
          casos: Math.max(prev.casos || 0, casos),
          bestDist: Math.min(bestPrev, best.d)
        });
      }
    }
    ...
    // ===== 4) Construir filas (formato actual) =====
    // TOTAL_SATELITES = total de satélites únicos dentro de ese buffer (según asignación final)
    const bufToAmies = new Map();
    for (const [amie, info] of chosen.entries()) {
      const key = String(info.alias);
      if (!bufToAmies.has(key)) bufToAmies.set(key, new Set());
      bufToAmies.get(key).add(amie);
    }

    const rows = [];
    // Orden por alias numérico
    const sortedAliases = Array.from(bufToAmies.keys()).sort((a,b)=>aliasNum(a)-aliasNum(b));

    for (const alias of sortedAliases) {
      const set = bufToAmies.get(alias);
      const total = set.size;
      const amiesSorted = Array.from(set).sort((a,b)=>String(a).localeCompare(String(b)));
      for (const amie of amiesSorted) {
        const info = chosen.get(amie);
        rows.push({
          BUFFER_ALIAS: alias,
    ...
    // Dedup final por seguridad
    const seen = new Set();
    return rows.filter(r => {
      const k = `${r.BUFFER_ALIAS}||${r.SATELITES_FISCALES}`;
    ...
    console.error('[EXPORT] buildBuffersBRowsForExport falló:', e);
    return [];
  }
}
...
function exportToExcel() {
  try {
    ...
    // Encabezados siempre
    const wsZonas = XLSX.utils.aoa_to_sheet([Object.keys((zonasRows && zonasRows[0]) || {
      CATEGORIA_MAPA:'', AMIE:'', NOMBRE:'', TIPO:'', SOSTENIMIENTO:'', PROVINCIA:'', CANTON:'', PARROQUIA:'',
      NUCLEO_ASIGNADO:'', DIST_KM:'', EN_BUFFER:'', BUFFER_ALIAS:''
    })]);
    if (zonasRows && zonasRows.length) {
      XLSX.utils.sheet_add_json(wsZonas, zonasRows, { origin: 'A2', skipHeader: true });
    }
    XLSX.utils.book_append_sheet(wb, wsZonas, 'Cobertura_Zonas');

    // 2) Hoja Buffers_B (satélites fiscales contenidos en Buffers B#)
    // Asegurar que los alias B# y coberturas estén actualizados antes de exportar
    try { updateOrphanBuffers(); } catch(e) {}
    try { recomputeOrphanBufferCoverage(); } catch(e) {}

    const bRows = buildBuffersBRowsForExport();

    // Crear hoja con encabezados SIEMPRE (evita hoja en blanco si bRows llega vacío por timing)
    const wsB = XLSX.utils.aoa_to_sheet([['BUFFER_ALIAS','SATELITES_FISCALES','TOTAL_SATELITES','CASOS']]);
    if (bRows && bRows.length) {
      XLSX.utils.sheet_add_json(wsB, bRows, { origin: 'A2', skipHeader: true });
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  (async () => {
    ...
    if (r.ok) {
      ...
      try {
        showNotification('📌 Estado OFICIAL aplicado. Todos verán el mismo mapa/exportación.', 'success');
      } catch(_) {}
    } else {
      console.log('ℹ️ Estado oficial no aplicado:', r.reason);
      // Si el archivo existe pero está desactivado (useOfficial=false), avisa claramente.
      if (String(r.reason || '').includes('useOfficial=false')) {
        try {
          showNotification('ℹ️ El archivo /data/state_official.json está en modo plantilla (useOfficial=false). Para publicar tu 44.88%: clic en "Publicar" → sube el state_official.json descargado reemplazando el de /data/ → commit/push y cambia STATE_VERSION.', 'info');
        } catch(_) {}
      }
...
// pero NO forzamos el color a rojo: el color depende únicamente de estar dentro/fuera de cualquier buffer visible.
  ...
  const marker = L.circleMarker([s.lat, s.lng], {
    ...
    // (Meta) usado para exportación ESPEJO de Buffers B#
    marker._deceAmie = String(s?.amie || '').trim();
    marker._orphanBid = orphanBid || null;
...
function initAMIESearchInline(){
  ...
  const run = () => {
    ...
    // (Nuevo) Buscar Buffers por alias (Ej: B108)
    if (/^B\d+$/i.test(amie)) {
      const alias = amie.toUpperCase();
      const buf = (customBuffers || []).find(b => String(b?.orphanAlias || '').toUpperCase() === alias);
      if (!buf) {
        alert('Buffer no encontrado: ' + alias);
        return;
      }
      try {
        map.fitBounds(buf.circle.getBounds(), { padding: [30, 30] });
        // Abrir popup del buffer
        if (typeof showBufferPopup === 'function') {
          showBufferPopup(buf, true);
        }
        // Flash (resalta 1.5s)
        const prev = { color: buf.circle.options.color, weight: buf.circle.options.weight };
        buf.circle.setStyle({ color: '#ffffff', weight: 5 });
        setTimeout(() => { try { buf.circle.setStyle(prev); } catch(_) {} }, 1500);
      } catch (e) {
        console.warn('No se pudo enfocar buffer', e);
      }
      return;
    }

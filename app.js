...
let forcedAssignments = new Map(); // si -> {ni, distance}
let orphanSatellites = new Set();
let orphanNucleos = new Set();
let coverageStats = {
  totalSatellites: 0,
  coveredNormal: 0,
  coveredForced: 0,
...
function exportToCSV() {
  if (!globalData) {
    showNotification("⚠️ No hay datos", "warning");
    return;
  }
  
  console.log("[INFO] Exportando con asignaciones forzadas...");
...
";
  ...
  satellites.forEach((sat, si) => {
    ...
    if (bestNi !== null) {
      ...
      csv += `SATELITE,${si},"${escapeCSV(sat.name)}","${escapeCSV(sat.dist)}",${sat.lat},${sat.lng},${bufferLat},${bufferLng},${bestDist.toFixed(2)},${sat.students || 0},${assignment}
`;
    }
  });
  ...
  selected.forEach(ni => {
    ...
    const bufferLat = bufferData?.currentPos?.lat || nucleo.lat;
    const bufferLng = bufferData?.currentPos?.lng || nucleo.lng;
    const status = orphanNucleos.has(ni) ? "HUERFANO" : "ACTIVO";
...
`;
  ...
  link.download = `dece_cobertura_total_${new Date().toISOString().split('T')[0]}.csv`;
  link.click();
  
  console.log("[OK] CSV exportado");
  showNotification(`📥 Exportado: ${coverageStats.totalCoveragePercent}% cobertura`, "success");
}

function escapeCSV(str) {
  if (!str) return "";
  return String(str).replace(/"/g, '""');
}","${b.bufferName}","${b.isCustom ? 'Personalizado' : 'Original'}",${b.centerLat.toFixed(6)},${b.centerLng.toFixed(6)},${b.radiusMeters},${b.totalAMIEs},${b.nucleosCount},${b.satellitesCount},${b.totalStudents}\n`;
  });
...
function toggleEditMode() {
  ...
  console.log("🔧 toggleEditMode:", editMode);
  ...
  if (editMode) {
    ...
    editableBuffers.forEach(data => { 
      if (data.circle._draggable) {
        data.circle._draggable.disable();
        data.circle._originalDragging = false;
      }
    });
    customBuffers.forEach(buffer => { 
      if (buffer.circle._draggable) {
        buffer.circle._draggable.disable();
        buffer.circle._originalDragging = false;
      }
    });
  }
}
...
function toggleDeleteMode() {
  ...
  console.log("🗑️ toggleDeleteMode:", deleteMode);
  ...
  if (deleteMode) {
    btn?.classList.add("active");
    showNotification("🗑️ PASO 1: Click en buffer (se pone rojo) ► PASO 2: Presiona SUPR o DELETE", "info");
...
function deleteSelectedBuffer() {
  ...
  if (target.isCustom) {
    ...
    if (index >= 0) {
      map.removeLayer(customBuffers[index].circle);
    ...
    const bufferData = editableBuffers.get(target.ni);
    if (bufferData && bufferData.circle) {
      map.removeLayer(bufferData.circle);
      editableBuffers.delete(target.ni);
      markAsChanged();
...
function makeBufferDraggable(circle, data, isCustom) {
  console.log("🔧 makeBufferDraggable - Haciendo arrastrable:", isCustom ? "custom" : "nucleo");
  ...
  function onMouseDown(e) {
    ...
    console.log("🖱️ MOUSEDOWN - Iniciando arrastre");
    ...
    // Activar listeners de movimiento
    map.on('mousemove', onMouseMove);
    map.on('mouseup', onMouseUp);
    
    // Cambiar cursor
    map._container.style.cursor = 'move';
  }
  
  // MOUSEMOVE - Mover círculo
  function onMouseMove(e) {
    if (!isDragging) return;
    
    // Mover el círculo a la posición del mouse
  ...
  function onMouseUp(e) {
    ...
    console.log("🖱️ MOUSEUP - Finalizando arrastre");
    isDragging = false;
    
    // Reactivar arrastre del mapa
    map.dragging.enable();
    ...
    // Si se movió, guardar cambios
    if (startPos && (startPos.lat !== newPos.lat || startPos.lng !== newPos.lng)) {
      console.log("✅ Buffer movido:", startPos, "→", newPos);
      
      data.currentPos = { lat: newPos.lat, lng: newPos.lng };
      if (!isCustom) data.adjusted = true;
      
      markAsChanged();
      showNotification("📍 Buffer movido", "success");
  ...
  circle._draggable = {
    enable: () => { circle.on('mousedown', onMouseDown); },
    disable: () => { 
  ...
  console.log("✅ Buffer configurado como arrastrable");
}
  
  // Crear draggable usando el elemento del canvas/SVG
  const element = circle._path || circle._renderer._container;
  
  // Método alternativo más robusto: usar eventos de Leaflet directamente
  circle._originalDragging = true;
  ...
  circle._draggable = { 
    enable: () => { circle._originalDragging = true; },
    disable: () => { 
      circle._originalDragging = false;
      map.off('mousemove', onMouseMove);
    }
  };
}
...
function onMapClickForAdd(e) {
  ...
  circle.on('click', (e) => {
    L.DomEvent.stopPropagation(e);
    if (deleteMode) {
      // Solo seleccionar, NO eliminar inmediatamente
      if (selectedDeleteTarget && selectedDeleteTarget.id === newBuffer.id) {
        // Ya estaba seleccionado, deseleccionar
        circle.setStyle({ color: '#a371f7', weight: 2, opacity: 0.7 });
        selectedDeleteTarget = null;
        showNotification("⚪ Buffer deseleccionado", "info");
      } else {
        // Deseleccionar anterior si existe
        if (selectedDeleteTarget && !selectedDeleteTarget.isCustom) {
          const prevData = editableBuffers.get(selectedDeleteTarget.ni);
          if (prevData) {
            prevData.circle.setStyle({ color: '#58a6ff', weight: 2, opacity: 0.6 });
          }
        } else if (selectedDeleteTarget && selectedDeleteTarget.isCustom) {
          const prevBuffer = customBuffers.find(b => b.id === selectedDeleteTarget.id);
          if (prevBuffer) {
            prevBuffer.circle.setStyle({ color: '#a371f7', weight: 2, opacity: 0.7 });
          }
        }
        
        // Seleccionar este
        selectedDeleteTarget = { circle, id: newBuffer.id, isCustom: true };
        circle.setStyle({ color: '#ff0000', weight: 4, opacity: 0.9, fillOpacity: 0.2 });
        showNotification("🎯 Buffer seleccionado. Presiona SUPR o DELETE para eliminar", "info");
      }
    } else if (!editMode) {
...
function drawBuffersEditable(nucleos, selected, nucleoStats) {
  ...
  selected.forEach(ni => {
    ...
    circle.on('click', (e) => { 
      ...
      if (deleteMode) {
        // Solo seleccionar, NO eliminar inmediatamente
        if (selectedDeleteTarget && selectedDeleteTarget.ni === ni) {
          // Ya estaba seleccionado, deseleccionar
          circle.setStyle({ color: '#58a6ff', weight: 2, opacity: 0.6 });
          selectedDeleteTarget = null;
          showNotification("⚪ Buffer deseleccionado", "info");
        } else {
          // Deseleccionar anterior si existe
          ...
          // Seleccionar este
          selectedDeleteTarget = { circle, ni, isCustom: false };
          circle.setStyle({ color: '#ff0000', weight: 4, opacity: 0.9, fillOpacity: 0.2 });
          showNotification("🎯 Buffer seleccionado. Presiona SUPR o DELETE para eliminar", "info");
        }
      } else if (!editMode) {
...
function restoreCustomBuffer(saved) {
  ...
  circle.on('click', (e) => { 
    ...
    if (deleteMode) {
      // Solo seleccionar, NO eliminar inmediatamente
      if (selectedDeleteTarget && selectedDeleteTarget.id === buffer.id) {
        // Ya estaba seleccionado, deseleccionar
        circle.setStyle({ color: '#a371f7', weight: 2, opacity: 0.7 });
        selectedDeleteTarget = null;
        showNotification("⚪ Buffer deseleccionado", "info");
        ...
        // Seleccionar este
        selectedDeleteTarget = { circle, id: buffer.id, isCustom: true };
        circle.setStyle({ color: '#ff0000', weight: 4, opacity: 0.9, fillOpacity: 0.2 });
        showNotification("🎯 Buffer seleccionado. Presiona SUPR o DELETE para eliminar", "info");
      }
    } else if (!editMode) {
      ...
      if (customBuffers.length + editableBuffers.size < MAX_BUFFERS) {
        ...
        circle.on('click', (e) => {
          ...
          if (deleteMode) {
            ...
            if (selectedDeleteTarget && selectedDeleteTarget.id === newBuffer.id) {
              // Ya estaba seleccionado, deseleccionar
              circle.setStyle({ color: '#3fb950', weight: 2, opacity: 0.7 });
              selectedDeleteTarget = null;
              showNotification("⚪ Buffer deseleccionado", "info");
            } else {
              // Deseleccionar anterior si existe
              if (selectedDeleteTarget && !selectedDeleteTarget.isCustom) {
                ...
                const prevBuffer = customBuffers.find(b => b.id === selectedDeleteTarget.id);
                if (prevBuffer) {
                  const prevColor = prevBuffer.circle.options.color === '#3fb950' ? '#3fb950' : '#a371f7';
                  prevBuffer.circle.setStyle({ color: prevColor, weight: 2, opacity: 0.7 });
                }
              }
              ...
              selectedDeleteTarget = { circle, id: newBuffer.id, isCustom: true };
              circle.setStyle({ color: '#ff0000', weight: 4, opacity: 0.9, fillOpacity: 0.2 });
              showNotification("🎯 Buffer optimizado seleccionado. Presiona SUPR o DELETE", "info");
            }
          } else if (!editMode) {
            showBufferPopup(newBuffer, true);
          }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ===== CONFIGURACIÓN DE BOTONES - VERSIÓN AGRESIVA =====
  ...
  setTimeout(() => {
    ...
    console.log("btnEdit encontrado:", !!btnEdit);
    console.log("btnAdd encontrado:", !!btnAdd);
    console.log("btnDelete encontrado:", !!btnDelete);
    
    if (btnEdit) {
      // Remover cualquier listener previo
      btnEdit.replaceWith(btnEdit.cloneNode(true));
      const newBtnEdit = document.getElementById("btnEditBuffers");
      ...
      console.log("✅ Editar configurado");
    } else {
    ...
    if (btnAdd) {
      ...
      const newBtnAdd = document.getElementById("btnAddBuffers");
      ...
      console.log("✅ Añadir configurado");
    } else {
    ...
    if (btnDelete) {
      ...
      const newBtnDelete = document.getElementById("btnDeleteBuffers");
      
      newBtnDelete.addEventListener("click", function(e) {
        console.log("🗑️ ¡CLICK EN ELIMINAR!");
        e.preventDefault();
        e.stopPropagation();
        toggleDeleteMode();
      });
      console.log("✅ Eliminar configurado");
    } else {
...
console.log("📦 DECE App v7.3 - DEFINITIVO - Arrastre y Eliminación FUNCIONANDO!");

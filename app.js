/*************************************************
 * DECE Coverage App - v6.1 OPTIMIZED 🚀
...
// ========== 🚀 OPTIMIZACIÓN: CACHÉ Y DEBOUNCING ==========
let orphanAnalysisCache = null;
let analyzeOrphansTimer = null;
let regenerateAnimationsTimer = null;
const DEBOUNCE_DELAY = 300; // ms - solo analizar después de 300ms sin movimiento
...
function deleteCustomBuffer(bufferId) {
  ...
  // 🚀 OPTIMIZACIÓN: Analizar después con debounce
  analyzeOrphansDebounced();
...
function disableBufferEditing() {
  // 🚀 OPTIMIZACIÓN: Ejecutar análisis final al terminar de editar
  if (analyzeOrphansTimer) {
    clearTimeout(analyzeOrphansTimer);
    analyzeOrphans();
  }
  ...
  circle.on('mousedown', function(e) {
    ...
    const onMove = (e) => {
      if (isDragging) {
        ...
        // 🚀 OPTIMIZACIÓN: No llamar a analyzeOrphans aquí
      }
    };
    ...
    const onUp = () => {
      ...
      // 🚀 OPTIMIZACIÓN: Solo analizar si el drag duró más de 100ms
      if (dragDuration > 100) {
        analyzeOrphansDebounced();
...
// ========== 🚀 REGENERACIÓN DE ANIMACIONES CON DEBOUNCE ==========
function regenerateAnimationsDebounced() {
  if (regenerateAnimationsTimer) {
    clearTimeout(regenerateAnimationsTimer);
  }
  
  regenerateAnimationsTimer = setTimeout(() => {
    regenerateAnimations();
    regenerateAnimationsTimer = null;
...
function regenerateAnimations() {
  ...
  satellites.forEach(sat => {
    ...
    editableBuffers.forEach((data) => {
      const pos = data.circle.getLatLng();
      const dist = getCachedDistance(sat.lat, sat.lng, pos.lat, pos.lng);
...
// ========== 🚀 FUNCIONES OPTIMIZADAS: analyzeOrphans con debounce ==========
function analyzeOrphansDebounced() {
...
function analyzeOrphans() {
  ...
  console.time("⚡ analyzeOrphans");
  console.log("=== ANÁLISIS DE HUÉRFANOS (OPTIMIZADO) ===");
...
const findClosestAnyWithin = (lat, lng, maxDistM) => {
  ...
  for (let dLat = -2; dLat <= 2; dLat++) {
    for (let dLng = -2; dLng <= 2; dLng++) {
      ...
      if (!arr) continue;
      for (const n of arr) {
        const dist = getCachedDistance(n.lat, n.lng, lat, lng);
...
const findClosestAnyOverall = (lat, lng) => {
  ...
  console.timeEnd("⚡ analyzeOrphans");
  
  // Guardar en caché
  orphanAnalysisCache = JSON.parse(JSON.stringify(orphanAnalysis));
  
  updateOrphanPanel();
  refreshSatellitesLayer();
  // 🚀 OPTIMIZACIÓN: Solo regenerar si estamos en modo edición
  if (editMode) {
    regenerateAnimationsDebounced();
  } else {
    regenerateAnimations();
  }
}

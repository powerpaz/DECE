...
function enableDragging(circle, data, isCustom) {
  ...
  if (circle._draggingEnabled) {
    console.log("⚠️ Dragging already enabled for this circle");
    return;
  }
  
  console.log("✅ Enabling dragging for circle"); // Debug
  
  let isDragging = false;
  let startLatLng = null;
  
  // Almacenar referencias a los handlers para poder removerlos después
  const mouseDownHandler = function(e) {
    if (!editMode) return;
    
    console.log("🖱️ Mouse down on buffer"); // Debug
    ...
    // Deshabilitar arrastre del mapa
  ...
  const mouseUpHandler = function(e) {
    ...
    console.log("🖱️ Mouse up - ending drag"); // Debug
    
    isDragging = false;
    
    // Re-habilitar arrastre del mapa
    map.dragging.enable();
...
function disableDragging(circle, data) {
  if (!circle._draggingEnabled) return;
  
  console.log("❌ Disabling dragging for circle"); // Debug
  
  // Remover event listeners
  if (circle._dragHandlers) {
    circle.off('mousedown', circle._dragHandlers.mouseDown);
    map.off('mousemove', circle._dragHandlers.mouseMove);
    map.off('mouseup', circle._dragHandlers.mouseUp);
...
function resolveColumnIndexes(headerRow) {
  console.log("🔍 Resolviendo columnas, header:", headerRow);
...
function processData(data) {
  console.log("⚙️ processData iniciado con", data.length, "registros");
...
function hideLoadingOverlay() { 
  ...
  const o = document.getElementById("loadingOverlay"); if (o) { o.style.opacity = "0"; setTimeout(() => o.style.display = "none", 500); } }

...
function toggleDeleteMode() {
  ...
  if (deleteMode) {
    btn?.classList.add("active");
    showNotification("🗑️ Modo eliminar: Click en buffer o presiona SUPR/DELETE", "info");
...
function handleDeleteKey(e) {
  ...
  if (e.key === 'Delete' || e.keyCode === 46 || e.key === 'Backspace' || e.keyCode === 8) {
    ...
    if (selectedDeleteTarget) {
      ...
      if (selectedDeleteTarget.isCustom) {
        deleteCustomBuffer(selectedDeleteTarget.id);
      } else {
        // Eliminar buffer editable
        if (confirm('¿Eliminar este buffer original?')) {
          map.removeLayer(selectedDeleteTarget.circle);
          editableBuffers.delete(selectedDeleteTarget.ni);
          markAsChanged();
          showNotification("🗑️ Buffer eliminado con tecla", "success");
...
function onMapClickForAdd(e) {
  ...
  circle.on('click', (e) => {
    ...
    if (deleteMode) {
      ...
      circle.setStyle({ color: '#ff0000', weight: 4 });
      showNotification("✅ Buffer seleccionado. Presiona SUPR/DELETE o haz click de nuevo", "info");
      
      // Si hace click de nuevo, eliminar directamente
      const index = customBuffers.findIndex(b => b.id === newBuffer.id);
      if (index >= 0 && confirm('¿Eliminar este buffer personalizado?')) {
        ...
        circle.setStyle({ color: '#a371f7', weight: 2 });
        selectedDeleteTarget = null;
      }
    } else if (!editMode) {
  ...
  selected.forEach(ni => {
    ...
    circle.on('click', (e) => { 
      ...
      if (deleteMode) {
        ...
        // Resaltar el buffer seleccionado
        circle.setStyle({ color: '#ff0000', weight: 4 });
        showNotification("✅ Buffer seleccionado. Presiona SUPR/DELETE o haz click de nuevo", "info");
        ...
        if (confirm('¿Eliminar este buffer original?')) {
          ...
          // Restaurar estilo
          circle.setStyle({ color: '#58a6ff', weight: 2 });
          selectedDeleteTarget = null;
        }
      } else if (!editMode) {
      ...
      if (customBuffers.length + editableBuffers.size < MAX_BUFFERS) {
        ...
        circle.on('click', (e) => {
          ...
          if (deleteMode) {
            selectedDeleteTarget = { circle, id: newBuffer.id, isCustom: true };
            circle.setStyle({ color: '#ff0000', weight: 4 });
            showNotification("✅ Buffer seleccionado. Presiona SUPR/DELETE", "info");
            
            const index = customBuffers.findIndex(b => b.id === newBuffer.id);
            if (index >= 0 && confirm('¿Eliminar este buffer?')) {
              map.removeLayer(circle);
              customBuffers.splice(index, 1);
              markAsChanged();
              showNotification("🗑️ Buffer eliminado", "success");
              ...
              circle.setStyle({ color: '#3fb950', weight: 2 });
              selectedDeleteTarget = null;
            }
          } else if (!editMode) {
            showBufferPopup(newBuffer, true);

...
function resolveColumnIndexes(headerRow) {
  ...
  const amie = findOne(["codamie", "cod_amie", "codigo_amie", "amie"]);
  const profs = findOne(["profesional", "profesionales", "prof", "dece"]);
  ...
  return {
    idx: { lat, lon, code, name, dist, students, amie, profs },
    issues
  };
}
...
function mapRowsToData(rows, idx) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const profs = idx.profs >= 0 ? parseNumberES(r[idx.profs]) : 0;

    data.push({ lat, lng, code: codeVal, name, dist, students, amie, profs });
...
function processData(data) {
  ...
  appState.satCandidates = satCandidates;
  ...
  appState.nucleoStats = nucleoStats;
...
/* ============================================================
 * BOTONES: ANÁLISIS ESPACIAL
 * - Detectar Densidad (urbano/rural)
 * - Buffers Dinámicos (<=7.5km)
 * - Spatial Join (asignación satélite -> núcleo seleccionado)
 * - Exportar (Excel/CSV)
 * ============================================================ */
...
function analyzeUrbanRuralDensity() {
  try {
    const nucleos = appState.nucleos || [];
    if (nucleos.length < 5) {
      showNotification("⚠️ No hay suficientes núcleos para analizar densidad", "warning");
      return;
    }

    // kNN simple (k=5): promedio de distancia a los 5 núcleos más cercanos
    const k = Math.min(5, Math.max(2, nucleos.length - 1));
    const avgDist = new Array(nucleos.length).fill(0);

    for (let i = 0; i < nucleos.length; i++) {
      const di = [];
      for (let j = 0; j < nucleos.length; j++) {
        if (i === j) continue;
        di.push(_haversineMeters(nucleos[i].lat, nucleos[i].lng, nucleos[j].lat, nucleos[j].lng));
      }
      di.sort((a, b) => a - b);
      const slice = di.slice(0, k);
      const mean = slice.reduce((s, v) => s + v, 0) / slice.length;
      avgDist[i] = mean;
    }

    // umbral: mediana (menor distancia = más denso = "urbano")
    const sorted = [...avgDist].sort((a, b) => a - b);
    const median = sorted[Math.floor(sorted.length / 2)];

    const densityClass = avgDist.map(d => (d <= median ? "URBANO" : "RURAL"));
    appState.densityClass = densityClass;

    // Visual: halo sutil en buffers para indicar densidad (sin tocar tamaño de núcleos)
    // Re-dibujar buffers existentes con color distinto según clase
    // Nota: los buffers se redibujan por drawBuffersEditable; aquí solo damos feedback.
    const urbanCount = densityClass.filter(x => x === "URBANO").length;
    const ruralCount = densityClass.length - urbanCount;

    showNotification(`📊 Densidad calculada: ${urbanCount} urbano / ${ruralCount} rural`, "success");
  } catch (e) {
    console.error(e);
    showNotification("❌ Error en Detectar Densidad (ver consola)", "error");
  }
}

function generateOptimalBuffers() {
  // Re-ejecuta la selección de buffers con la data actual
  try {
    if (!appState.data || !appState.data.length) {
      showNotification("⚠️ No hay datos cargados todavía", "warning");
      return;
    }
    showLoadingOverlay("Recalculando buffers…", "Optimizando cobertura 95–98%");
    // Re-procesa data completa (mantiene la política actual COVER/USED/TOP)
    setTimeout(() => {
      processData(appState.data);
      showNotification("✅ Buffers optimizados (solo los necesarios)", "success");
    }, 50);
  } catch (e) {
    console.error(e);
    showNotification("❌ Error optimizando buffers", "error");
  }
}
...
function executeSpatialJoin() {
  try {
    ...
    const satCandidates = appState.satCandidates || [];
    const selected = appState.selected || new Set();

    if (!nucleos.length || !satellites.length) {
      showNotification("⚠️ No hay núcleos/satélites para Spatial Join", "warning");
      return;
    }
    if (!selected.size) {
      showNotification("⚠️ No hay núcleos seleccionados (buffers) para el join", "warning");
      return;
    }

    // Construye asignación: cada satélite -> núcleo seleccionado más cercano (dentro de 7.5km)
    const detail = [];
    const byNucleo = new Map(); // ni -> array sat idx
    const uncovered = [];
    ...
    for (let si = 0; si < satellites.length; si++) {
      ...
      for (let c = 0; c < cands.length; c++) {
        const cand = cands[c];
        if (!selected.has(cand.ni)) continue;
      ...
      if (!byNucleo.has(best.ni)) byNucleo.set(best.ni, []);
      byNucleo.get(best.ni).push(si);

      const n = nucleos[best.ni];
      detail.push({
        NUCLEO_AMIE: n.amie || "",
        NUCLEO_NOMBRE: n.name || "",
        NUCLEO_DISTRITO: n.dist || "",
        NUCLEO_LAT: n.lat,
        NUCLEO_LON: n.lng,
        NUCLEO_COD_GDECE: n.code,
        SAT_AMIE: s.amie || "",
        SAT_NOMBRE: s.name || "",
        SAT_DISTRITO: s.dist || "",
        SAT_LAT: s.lat,
        SAT_LON: s.lng,
        SAT_COD_GDECE: s.code,
        DIST_KM: +(best.d / 1000).toFixed(3),
        ESTUDIANTES_SAT: s.students || 0
      });
    }

    // Resumen por núcleo
    const summary = [];
    for (const [ni, satIdxs] of byNucleo.entries()) {
      const n = nucleos[ni];
      const totStudents = (n.students || 0) + satIdxs.reduce((acc, si) => acc + (satellites[si].students || 0), 0);
      const profNecesarios = Math.ceil(totStudents / 450);
      summary.push({
        NUCLEO_AMIE: n.amie || "",
        NUCLEO_NOMBRE: n.name || "",
        NUCLEO_DISTRITO: n.dist || "",
        ...
        NUCLEO_LON: n.lng,
        NUCLEO_COD_GDECE: n.code,
        SAT_ASIGNADOS: satIdxs.length,
        ESTUDIANTES_TOTAL: totStudents,
        PROF_ACTUALES: n.profs || 0,
        PROF_NECESARIOS: profNecesarios,
        DEFICIT: profNecesarios - (n.profs || 0)
      });
    }
    summary.sort((a, b) => b.SAT_ASIGNADOS - a.SAT_ASIGNADOS);

    appState.joinDetail = detail;
    appState.joinSummary = summary;

    // UI: refresca Top10 desde join (más representativo)
    const top10 = summary.slice(0, 10);
    const html = top10.map((r, i) => {
      const deficitColor = r.DEFICIT > 0 ? "#f85149" : "#3fb950";
      return `
        <div class="top-item" onclick="flyToLocation(${r.NUCLEO_LAT}, ${r.NUCLEO_LON})">
          <div class="top-item-header">
            <span class="top-rank">#${i + 1}</span>
            <span class="top-name">${(r.NUCLEO_NOMBRE || "Núcleo").slice(0, 38)}</span>
            <span class="top-count">${r.SAT_ASIGNADOS}</span>
          </div>
          <div class="top-desc">
            ${Number(r.ESTUDIANTES_TOTAL || 0).toLocaleString()} estudiantes • Déficit <span style="color:${deficitColor}">${r.DEFICIT}</span>
          </div>
        </div>
      `;
    }).join("");
    const topEl = document.getElementById("topNucleos");
    if (topEl) topEl.innerHTML = html;

    const coverage = ((satellites.length - uncovered.length) / satellites.length) * 100;
    showNotification(`✅ Spatial Join listo: ${detail.length} enlaces. Cobertura: ${coverage.toFixed(1)}%`, "success");
  } catch (e) {
    console.error(e);
    showNotification("❌ Error ejecutando Spatial Join (ver consola)", "error");
  }
}
...
function _exportExcel(rows, filename) {
  if (!window.XLSX) {
    ...
    _downloadText(filename.replace(/\.xlsx$/i, ".csv"), csv);
    showNotification("ℹ️ No se cargó XLSX, exporté CSV como fallback", "warning");
    return;
  }
...
function showExportMenu() {
  try {
    const detail = appState.joinDetail || [];
    const summary = appState.joinSummary || [];
    if (!detail.length && !summary.length) {
      showNotification("⚠️ Primero ejecuta Spatial Join para generar resultados", "warning");
      return;
    }
    const ts = new Date().toISOString().slice(0,19).replace(/[:T]/g,'-');
    // Exporta ambos (detalle + resumen)
    if (summary.length) _exportExcel(summary, `DECE_spatial_join_resumen_${ts}.xlsx`);
    if (detail.length) _exportExcel(detail, `DECE_spatial_join_detalle_${ts}.xlsx`);
    showNotification("📦 Exportación lista (resumen + detalle)", "success");
  } catch (e) {
    console.error(e);

...
function buildBuffersBRowsForExport() {
  try {
    ...
    const aliasById = new Map();
    list.forEach(ob => {
      aliasById.set(ob.bufferId, String(ob.alias || ob.bufferId));
    });

    // ESPEJO del mapa (y calidad):
    // - Un AMIE debe pertenecer a UN SOLO Buffer B# (aunque el círculo se solape con varios)
    // - Guardar CASOS = cantidad de buffers candidatos donde cayó ese AMIE
    // - Asignación determinística: buffer candidato más cercano al punto (centro del buffer)
    const bufToSet = new Map();     // bufferId -> Set<AMIE>
    const bufToCasos = new Map();   // bufferId -> Map<AMIE, CASOS>

    // index de centros de buffers B#
    const centerById = new Map(); // bufferId -> {lat,lng}
    list.forEach(ob => {
      const lat = Number(ob.centerLat);
      const lng = Number(ob.centerLng);
      if (Number.isFinite(lat) && Number.isFinite(lng)) centerById.set(ob.bufferId, { lat, lng });
    });

    // recolectar candidatos por AMIE desde la capa visible
    const candidatesByAmie = new Map(); // AMIE -> [{bid, distM}]
    try {
      if (layers?.satellitesOrphanBuffers && typeof layers.satellitesOrphanBuffers.eachLayer === 'function') {
        layers.satellitesOrphanBuffers.eachLayer((lyr) => {
          const bid = lyr?._orphanBid;
          const amie = String(lyr?._deceAmie || '').trim();
          const ll = lyr?.getLatLng?.();
          if (!bid || !amie || !ll) return;
          const c = centerById.get(bid);
          if (!c) return;

          // distancia (usar Leaflet si existe; fallback a haversine)
          let distM = 0;
          try {
            if (typeof map !== 'undefined' && map?.distance) distM = map.distance(ll, c);
            else {
              const toRad = (x) => (x * Math.PI) / 180;
              const R = 6371000;
              const dLat = toRad(c.lat - ll.lat);
              const dLng = toRad(c.lng - ll.lng);
              const a = Math.sin(dLat/2)**2 + Math.cos(toRad(ll.lat))*Math.cos(toRad(c.lat))*Math.sin(dLng/2)**2;
              distM = 2 * R * Math.asin(Math.sqrt(a));
            }
          } catch(_) { distM = 0; }

          if (!candidatesByAmie.has(amie)) candidatesByAmie.set(amie, []);
          candidatesByAmie.get(amie).push({ bid, distM });
        });
      }
    } catch (e) {
      console.warn('[WARN] No se pudo leer capa satellitesOrphanBuffers para exportar Buffers_B:', e);
    }

    // resolver asignación única por AMIE
    const aliasNum = (a) => {
      const m = String(a || '').match(/(\d+)/);
      return m ? parseInt(m[1], 10) : 0;
    };
    const pickBest = (amie, arr) => {
      const uniq = new Map(); // bid -> minDist
      (arr || []).forEach(({bid, distM}) => {
        if (!bid) return;
        const prev = uniq.get(bid);
        if (prev == null || distM < prev) uniq.set(bid, distM);
      });
      const bids = Array.from(uniq.entries()).map(([bid, distM]) => ({ bid, distM }));
      bids.sort((x, y) => {
        // 1) menor distancia
        if (x.distM !== y.distM) return x.distM - y.distM;
        // 2) alias menor (B#)
        const ax = aliasNum(aliasById.get(x.bid));
        const ay = aliasNum(aliasById.get(y.bid));
        if (ax !== ay) return ax - ay;
        // 3) bufferId string
        return String(x.bid).localeCompare(String(y.bid));
      });
      return bids[0]?.bid || null;
    };

    candidatesByAmie.forEach((cand, amie) => {
      const casos = (cand || []).length;
      const chosenBid = pickBest(amie, cand);
      if (!chosenBid) return;
      if (!bufToSet.has(chosenBid)) bufToSet.set(chosenBid, new Set());
      bufToSet.get(chosenBid).add(amie);
      if (!bufToCasos.has(chosenBid)) bufToCasos.set(chosenBid, new Map());
      bufToCasos.get(chosenBid).set(amie, casos);
    });
    ...
    // Salida VERTICAL (espejo): BUFFER_A | SATELITES | TOTAL_SATELITES | CASOS
    for (const ob of sorted) {
      const alias = String(ob.alias || ob.bufferId);
      const set = bufToSet.get(ob.bufferId) || new Set();
      const casosMap = bufToCasos.get(ob.bufferId) || new Map();
      const uniq = Array.from(set).filter(Boolean).sort();
      const total = uniq.length;

      if (!uniq.length) {
        rows.push({ BUFFER_A: alias, SATELITES: '', TOTAL_SATELITES: 0, CASOS: 0 });
        continue;
      }

      uniq.forEach((amie) => {
        rows.push({ BUFFER_A: alias, SATELITES: amie, TOTAL_SATELITES: total, CASOS: Number(casosMap.get(amie) || 1) });
      });
    }

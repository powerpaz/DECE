/*************************************************
 * DECE Coverage App - v7.0 ULTRA OPTIMIZED 🚀
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  console.log('[INIT] Iniciando aplicación DECE Optimizer v7.0');
...
function loadCSV() {
  ...
  fetch('DECE_CRUCE_X_Y_NUC_SAT.csv', { cache: 'no-store' })
    ...
    .then(rawText => {
      ...
      console.log('[CSV] Primera línea:', firstLine.substring(0, 150));
...
function resolveColumnIndexes(headerRow) {
  ...
  console.log('[HEADERS] Primeras 10 columnas:', header.slice(0, 10));
  
  const find = (candidates) => {
    for (let c of candidates) {
      const idx = header.findIndex(h => h.includes(c));
      if (idx >= 0) {
        console.log(`[COLUMN] Encontrada "${c}" en índice ${idx}`);
        return idx;
      }
    }
    console.warn(`[COLUMN] No encontrada ninguna de:`, candidates);
    return -1;
  };
...
function mapRowsToData(rows, idx) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (!Number.isFinite(lat) || !Number.isFinite(lng) || !Number.isFinite(typeCode)) {
      invalidCount++;
      continue;
    }
  ...
  console.log(`[DATA] Válidos: ${validCount}, Inválidos: ${invalidCount}`);
...
function processData(data) {
  ...
  if (savedState?.customBuffers) {
    ...
    console.log(`[RESTORE] ${savedState.customBuffers.length} buffers personalizados restaurados`);
  }
  
  globalData = {
    nucleos,
    satellites,
    allInstitutions: [...nucleos, ...satellites]
  };
  ...
  hideLoadingOverlay();
  showNotification(`✅ ${nucleos.length} núcleos y ${satellites.length} satélites cargados`, 'success');
}
...
function deleteCustomBuffer(id) {
  ...
  console.log('[BUFFER] Buffer personalizado eliminado:', id);
...
console.log('[APP] DECE Optimizer v7.0 cargado correctamente ✅');
